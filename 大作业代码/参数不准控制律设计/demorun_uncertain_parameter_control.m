...
function dx = closed_loop_rhs(t, x, inertial_true, inertial_hat, ctrl)
    ...
    tau_raw = stable_controller(t, q, dq, inertial_hat, ctrl);
...
function assert_runtime_ok(info)
    if ~info.is_finite
...
function report_runtime_state(t, x, inertial_true, inertial_hat, ctrl)
    q = x(1:7);
    dq = x(8:14);
    [tau_raw, parts] = stable_controller(t, q, dq, inertial_hat, ctrl);
    tau = limit_tau(tau_raw, ctrl.tau_limit);
    [M, C, G] = gen3_lagrange_dynamics_param(q, dq, zeros(7, 1), inertial_true);
    rhs = tau - C * dq - G;
    M_solve = regularize_mass_matrix(M, ctrl);
