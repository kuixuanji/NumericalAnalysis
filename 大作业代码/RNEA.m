...
N = 7;
    ...
    R_rel = zeros(3,3,N);% 存储连杆间的相对旋转矩阵
    P_rel = zeros(3,N);% 存储连杆间的相对平移向量
    ...
    for i = 1:size(MDH_params,1)
       ...
       % 只有前 7 个连杆需要计算运动学状态
        if i <= N
            % 1. 角速度
            w(:,i) = R_to_curr * w_prev + Z_axis * dq(i);

            % 2. 角加速度
            dw(:,i) = R_to_curr * dw_prev + cross(R_to_curr * w_prev, Z_axis * dq(i)) + Z_axis * ddq(i);

            % 3. 线加速度 (原点加速度)
            P_i = P_in_prev(:,i);
            dv(:,i) = R_to_curr * (dv_prev + cross(dw_prev, P_i) + cross(w_prev, cross(w_prev, P_i)));
